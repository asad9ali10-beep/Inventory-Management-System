...
void menu_add_product()
{
    ...
    if (choice == 1)
    {
        ...
        cin >> warranty;
        cout << "Enter Fragility Rating (1-10): ";
        cin >> frag_rating;
        cin.ignore();
        cout << "Enter Packaging Type: ";
        getline(cin, pkg_type);
    ...
    else if (choice == 4)
    {
        ...
        cout << "Is Halal? (1=Yes, 0=No): ";
        cin >> halal_int;
        cout << "Enter Shelf Life (years): ";
        cin >> shelf_years;
        cin.ignore();
        is_halal_input = (halal_int == 1);
        cout << "Enter Preservative Level (Low/Medium/High): ";
        getline(cin, pres_level);
    ...
    else if (choice == 5)
    {
        ...
        cout << "Enter Fabric         : ";
        getline(cin, fabric);
        cout << "Enter Gender (Male/Female/Unisex): ";
        getline(cin, gender);
...
void menu_compare_products()
{
    ...
    for (int i = 0; i < total_products; i++)
    {
        if (all_products[i]->get_product_id() == pid1)
            p1 = all_products[i];
        if (all_products[i]->get_product_id() == pid2)
            p2 = all_products[i];
    }
...
void menu_add_supplier()
{
    ...
    getline(cin, sname);
    cout << "Enter Contract Terms    : ";
    getline(cin, terms);
    cout << "Enter Linked Product ID : ";
    getline(cin, prod_id);
...
void menu_transaction_log_demo()
{
    ...
    {
        string action = "Product added: " + all_products[i]->get_product_name() + " | ID: " + all_products[i]->get_product_id();
        product_log.record_action(action, *all_products[i]);
    }

    for (int i = 0; i < total_suppliers && i < 3; i++)
    {
        string action = "Supplier registered: " + all_suppliers[i].get_supplier_name() + " | ID: " + all_suppliers[i].get_supplier_id();
...
int main()
{
    ...
    while (!logged_in)
    {
        ...
        if (username == "invent" && password == "invent1122")
        {
    ...
    while (choice != 9)
    {
        ...
        cout << "4. Compare Two Products " << endl;
        cout << "5. Add Supplier" << endl;
        cout << "6. Display All Suppliers" << endl;
        cout << "7. Warehouse Demo " << endl;
        cout << "8. Transaction Log Demo " << endl;
        ...
        if (choice == 1)
            ...
            menu_display_all_suppliers();
        else if (choice == 7)
            menu_warehouse_demo();
        else if (choice == 8)
            menu_transaction_log_demo();
        else if (choice == 9)
        {
            save_all_data();
